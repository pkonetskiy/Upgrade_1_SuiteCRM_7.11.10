...
<table id="table_collection_{$vardef.name}" style="border-collapse:separate;border:solid 1px #fff; margin-top: 5px !important;border-radius: 6px;">
        ...
        <tr id="lineFields_{$displayParams.formName}_{$vardef.name}_{$extra_value}">
            {foreach item=extra_field from=$displayParams.to_display.$extra_value key=key_extra}
                {if $extra_field.hidden != 'hidden'}
                    <td nowrap style="padding: 5px 5px 10px 5px !important; vertical-align: middle;text-align: left;">
                        {$extra_field.field}
                    </td>
                {/if}
            {/foreach}
